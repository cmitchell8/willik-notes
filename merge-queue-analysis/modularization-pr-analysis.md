...
Even if every possible modularization-related PR is excluded, the effort rate moves from 8.0% to 8.8% — a negligible change that does not alter any conclusions from the [merge queue separation analysis](merge-queue-separate-queues-analysis.md).
...
- Title or body contains modularization keywords: "modulariz\*", "move to package", "extract to package", "split package", "reorganize package", "migrate to package", "create package", or "new package" (case-insensitive substring matches)
...
To confirm the classifier's accuracy, we manually reviewed the most recent 1,000 PRs (95 YES, 760 NO, 145 ambiguous), examining title, branch name, body text, and changed-file counts for each.
...
1. **Effort rate barely moves**: 8.0% → 8.8% is a +0.8pp change. Cross-repo efforts remain a small minority of queue volume regardless.
...
| Script | Purpose |
|--------|---------|
| `classify_modularization_prs.py` | Main classifier: fetches PRs via GitHub GraphQL/REST, applies deterministic rules, outputs ambiguous PRs for review, generates report |
| `worst_case_check.py` | Computes queue statistics assuming all ambiguous PRs are modularization |
| `validate_classification.py` | Exports recent PRs with bucket labels for manual or LLM validation |
